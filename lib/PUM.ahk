...
{   
  static _defaults := { "tcolor"     : pumAPI.GetSysColor( 7 ) ;default - COLOR_MENUTEXT
                        ...
                        ,"xmargin"   : 3
                        ,"ymargin"   : 3
                        ,"textMargin" : 5 } ;this is a pixels zmount which will be added after the text to make menu look pretty
  ...
  Show( x = 0, y = 0, flags = "" )
  {
    ...
    if !isContext ;check if menu is not recursed menu ( has TPM_RECURSE flag )
    {
      ...
      ;GUI PUM_MENU_GUI: Add, Edit, y-10 w200 h24
  ...
  {
    ; 防止出现两横（分割符）
    If not this._Separator And not params
      return
    this._Separator := params
    if !this.alive
      return 0
...
PUM_OnMeasure( wParam, lParam, msg, hwnd )
{
  ...
  NumPut( item.disabled=2?0:h, lParam+0, 16, "UInt" )
  return True
}
